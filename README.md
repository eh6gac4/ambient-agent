...
**hourly_gmail の詳細:**
- 未読メールを要約し、タスクストアに **1 メール = 1 タスク** で登録（サブタスクには分割しない。返信スレッドは新規タスクを作らず既存タスクの Priority / Due 更新と本文追記のみ）
...
**サブタスク化:** 画像からタスクを生成する際は、Gemini が抽出した各アクションを本文内のチェックボックスではなく **独立したタスク（親への `parent` リレーション付き）** として作成し、それぞれ Due / Priority / アイコンを個別に持たせる。親タスクは要約・本文（Markdown）を保持。
